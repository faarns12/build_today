...
const services: Service[] = [
  {
    ...
    badges: [
      { icon: <Hammer className="h-4 w-4" />, text: "200+ units" },
      { icon: <Ruler className="h-4 w-4" />, text: "Modern layouts" },
      { icon: <Building2 className="h-4 w-4" />, text: "Investor-friendly" },
    ],
    desc:
  ...
  },
  {
    ...
    badges: [
      { icon: <Ruler className="h-4 w-4" />, text: "Turn-key" },
      { icon: <Building2 className="h-4 w-4" />, text: "Curated blocks" },
    ],
  ...
  },
  {
    ...
    badges: [
      { icon: <Hammer className="h-4 w-4" />, text: "Complex builds" },
      { icon: <Ruler className="h-4 w-4" />, text: "Efficient footprints" },
    ],
  ...
  },
  {
    ...
    img: "/img/location2.png",
    badges: [{ icon: <Building2 className="h-4 w-4" />, text: "End-to-end" }],
    desc: "Planning, services, approvals, and civil works handled in one team.",
  },
  {
    title: "Knockdown & Rebuild",
    img: "/img/location1.png",
    badges: [{ icon: <Hammer className="h-4 w-4" />, text: "Hassle-free" }],
...
export default function ServicesHoverSection() {
  ...
  return (
    <section className="mx-auto my-6  w-11/12 rounded-[26px] bg-[#0E2A44] text-white p-5 sm:p-8 md:p-12 relative overflow-hidden">
...
{services.map((s, i) => (
  <div
    ...
    onFocus={() => setActive(i)}
    className={[
      "relative group cursor-pointer overflow-hidden transition-all duration-500 ease-in-out ",
  ...
  >
    ...
    <div className="flex items-center justify-between hover:bg-[#29799E] px-4  rounded-[25px] w-full h-full ">
      ...
      <div className="flex flex-col justify-center min-w-0 h-full">
        <h3 className="text-xl md:text-2xl py-3 font-medium text-white">
          {s.title}
        </h3>
        ...
        >
          {s.desc && (
            <p className="mb-3 hidden md:block transition-all duration-500">
              {s.desc}
            </p>
          )}

          {s.badges && (
            <div className="hidden md:flex flex-wrap gap-3 transition-all duration-500">
              {s.badges.map((b, idx) => (
                <span
                  key={idx}
    ...
    {s.img && (
      <div
        className={[
          "pointer-events-none absolute right-0 top-1/2 hidden -translate-y-1/2 md:block ",
          "transition-all duration-300",
          active === i ? "opacity-100 translate-x-0" : "opacity-0 translate-x-6",
        ].join(" ")}
      >
        <div className="relative h-48 w-40 -rotate-6 mr-30 overflow-hidden rounded-2xl shadow-2xl ring-1 ring-white">
          <Image
            src={s.img}
            alt={s.title}
            fill
